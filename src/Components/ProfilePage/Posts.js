...
import { getUserDetails, getUserDetailsById, getUserPosts, uploadUserPost } from '../Files/User_profile_avator';
          ...
          preview={{
            toolbarRender: (
              ...
              <Space size={12} className="toolbar-wrapper">
                ...
                <Delete sx={{width:15,height:15,cursor:'pointer'}} onClick={()=>{deletePost(item.post)}}/>
              </Space>
            ),
          }}
        />
      ...
      <Fade in={uploadf}>
        <Box sx={style}>
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            Preview
            <img src={preview} style={{height:'200px',width:'200px',objectFit:'cover'}}/>
            <TextField value={caption} onChange={(e)=>setCaption(e.target.value)} label="caption"/>
            <LoadingButton variant='contained' loading={uploadingProcess} loadingIndicator={<>Uploading....</>} onClick={()=>handleUpload()} disabled={!uploadPost}>
              Upload
            </LoadingButton>
          </div>
     </Box>
